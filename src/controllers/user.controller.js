...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      new: true, //after updation new value will be returned in 'user'
    }
  ).select("-password");
  return res
    .status(200)
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  const avatarLocalPath = req.file?.path; //only one file so; but while registering we where uploading 2 img so .files
...
//check if user already exist -> username/email se
//4.check for avatar, images
//upload them to cloudinary, avatar
//create user objects --> create an entry in db
